"""
Smart Router for Qianji AI with Identity Fix
Routes queries to appropriate handlers based on content type
Ensures Qwen Max always identifies as Qianji (Qji), not Qwen
"""
import re
from .qwen_max_identity_fixed import call_qwen_max_with_identity

class SmartRouterIdentity:
    def __init__(self):
        self.date_keywords = [
            '今天', '今日', '现在', '当前', '日期', '日子', 
            '农历', '阴历', '阳历', '公历', '正月', '腊月',
...
如果您需要更详细的黄历信息（宜忌、冲煞、吉时等），我可以为您提供专业分析。"""
    ...
    def handle_general_query(self, message: str, conversation_history=None) -> str:
        """Handle general queries with Qwen Max Thinking mode and correct identity"""
        return call_qwen_max_with_identity(message, conversation_history, enable_search=True)