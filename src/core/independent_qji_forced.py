"""
Independent Qji Max Fusion Engine for Qianji AI - Forced Date Validation Version
Combines Qwen Max base model with forced date validation for accuracy
"""
from .independent_qwen import call_qwen_max_api
from .forced_date_validator import ForcedDateValidator
import json
from datetime import datetime

class IndependentQjiEngine:
    def __init__(self):
        """Initialize Qji Max fusion engine with forced date validation"""
        print("🚀 正在初始化独立Qji Max引擎（强制日期验证版）...")
        self.qwen_engine = call_qwen_max_api
        self.date_validator = ForcedDateValidator()
        print("✅ 独立Qji Max引擎（强制日期验证版）初始化完成！")
    
    def generate_response(self, message, conversation_history=None):
        """
        Generate response using Qwen Max + forced date validation
        ...
        # Check if this message requires forced date validation
        if self.date_validator.should_force_date_validation(message):
            print(f"🔍 检测到日期相关查询，启用强制日期验证: {message}")
            return self.date_validator.generate_accurate_date_response(message)
        
        # Get current date context for general queries
        now = datetime.now()
...
- 如果用户的问题涉及日期，请务必使用当前真实日期：{current_date}
...
"""
    ...
    def analyze_bazi(self, birth_date, birth_time, gender, location):
        ...
        prompt = f"""
当前日期：{now.strftime('%Y年%m月%d日')}，{["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"][now.weekday()]}
